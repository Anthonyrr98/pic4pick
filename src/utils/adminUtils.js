...
import { UPLOAD_TYPES, resolveOssBackendApiUrl } from './upload';
...
export const deleteOSSFile = async (url) => {
  ...
  const getBackendBaseUrl = () => {
    const uploadApi = resolveOssBackendApiUrl('/api/upload/oss');
    if (uploadApi) {
      if (uploadApi.startsWith('http://') || uploadApi.startsWith('https://')) {
        try {
          const parsed = new URL(uploadApi);
